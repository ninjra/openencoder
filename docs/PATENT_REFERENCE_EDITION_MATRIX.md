...
This matrix is the public-safe burn-down for making OpenEncoder a zero-shot,
...
| 04 | Groth16 BN254 topology proof       | Implemented                   | One reference topology lane           |
...
| 11 | Groth16 BN254 verifier              | Implemented                   | Keep valid/tamper fixtures    |
| 12 | Fidelity ladder                      | Not implemented               | Add only when tested          |
| 13 | OpenEncoder APE                   | Multi-OS command shell        | Keep Python as source truth   |
+----+--------------------------------------+-------------------------------+-------------------------------+
```
...
generation, and Groth16 support is implemented for the reference topology proof
...
| Groth16 BN254 topology lane          | Implemented for reference emit/decode compatibility gating   |
...
receipt model, compatibility gates, Groth16 BN254 verifier and replay receipts are implemented and tested
for the client reference boundary. OpenEncoder generates one deterministic reference BN254 topology proof, but does not ship a general circuit compiler, reusable trusted setup ceremony, or Plonk verification.
