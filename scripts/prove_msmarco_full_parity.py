...
def prove(args: argparse.Namespace) -> dict[str, Any]:
    ...
    parity = 100.0 if totals["encoded_sources"] and mismatch_count == 0 else (1.0 - (mismatch_count / max(1, totals["encoded_sources"]))) * 100.0
    artifact = {
        "schema_version": SCHEMA_VERSION,
        "proof_passed": proof_passed,
        "claim_boundary": "OpenEncoder encode/decode parity over every text item in the local MS MARCO Arrow cache; no retrieval-ranking or service-quality claim; legacy BN254 pairing fixture is covered by docs/proofs/groth16_verification_proof.json and the pinned real Groth16 reference circuit proof is covered by docs/proofs/openencoder_real_groth16_circuit_manifest.json.",
        ...
        "encode_decode_parity_percent": round(parity, 12),
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "encode_decode_parity_percent": artifact["encode_decode_parity_percent"],
