...
"""Package an portable launcher APE from an existing envelope prefix and fresh payloads."""
...
PAYLOADS = (
    ("OpenEncoderLauncher.exe", 0o644),
    ("OpenEncoderLauncher-linux-x86_64", 0o755),
    ("OpenEncoderLauncher-macos-x86_64", 0o755),
)
...
def main() -> int:
    ...
    payload_paths = {
        "OpenEncoderLauncher.exe": args.windows,
        "OpenEncoderLauncher-linux-x86_64": args.linux,
        "OpenEncoderLauncher-macos-x86_64": args.macos,
    }
    ...
    checks = {
        ...
        "windows_payload_matches": zip_hashes.get("OpenEncoderLauncher.exe") == sha256_file(args.windows),
        "linux_payload_matches": zip_hashes.get("OpenEncoderLauncher-linux-x86_64") == sha256_file(args.linux),
        "macos_payload_matches": zip_hashes.get("OpenEncoderLauncher-macos-x86_64") == sha256_file(args.macos),
    }
    receipt = {
        "schema_version": "openencoder-portable-launcher-ape-package-v1",
        ...
        "claim_boundary": "Reuses an existing portable launcher APE envelope prefix and replaces only deterministic embedded payload ZIP entries.",
    }
