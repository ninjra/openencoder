...
The current checked path verifies BN254 pairing receipts and legacy field-envelope
fixtures. Production proof claims require a pinned real circuit manifest,
trusted setup artifacts, and prover/verifier receipts; the legacy fixture alone
is intentionally release-blocking.
"""
...
GROTH16_ENVELOPE_PROOF_SCHEMA_VERSION = "openencoder-groth16-envelope-proof-v1"
GROTH16_ENVELOPE_CIRCUIT_ID = "openencoder-field-envelope-relation-v1"
REAL_CIRCUIT_SCHEMA_VERSION = "openencoder-real-groth16-circuit-manifest-v1"
REAL_CIRCUIT_MANIFEST_PATH = Path("docs/proofs/openencoder_real_groth16_circuit_manifest.json")
...
def _load_py_ecc() -> dict[str, Any]:
    try:
        from py_ecc.bn128 import FQ, FQ2, FQ12, G1, G2, add, b, b2, curve_order, field_modulus, is_on_curve, multiply, neg, pairing
    except Exception as exc:  # pragma: no cover - exercised when optional extra is absent
        raise Groth16VerificationError(
            "py-ecc is required for Groth16 field-envelope proofs; install the OpenEncoder package"
        ) from exc
    return {
        "FQ": FQ,
        "FQ2": FQ2,
...
def _parse_g1(value: Any, *, label: str, ecc: dict[str, Any]) -> Any:
    ...
    b = ecc["b"]
    field_modulus = ecc["field_modulus"]
    curve_order = ecc["curve_order"]
    multiply = ecc["multiply"]
    point = _require_sequence(value, label=label, min_len=2)
    x = _as_int(point[0], label=f"{label}_x")
    y = _as_int(point[1], label=f"{label}_y")
    if x < 0 or x >= field_modulus or y < 0 or y >= field_modulus:
        raise Groth16VerificationError(f"{label}_coordinate_non_canonical")
    parsed = (FQ(x), FQ(y))
    if not is_on_curve(parsed, b):
        raise Groth16VerificationError(f"{label}_not_on_g1")
    if multiply(parsed, curve_order) is not None:
        raise Groth16VerificationError(f"{label}_not_in_g1_subgroup")
    return parsed
...
def _parse_g2(value: Any, *, label: str, ecc: dict[str, Any]) -> Any:
    ...
    coords = [
        _as_int(x[0], label=f"{label}_x0"),
        _as_int(x[1], label=f"{label}_x1"),
        _as_int(y[0], label=f"{label}_y0"),
        _as_int(y[1], label=f"{label}_y1"),
    ]
    if any(coord < 0 or coord >= field_modulus for coord in coords):
        raise Groth16VerificationError(f"{label}_coordinate_non_canonical")
    parsed = (
        FQ2([coords[0], coords[1]]),
        FQ2([coords[2], coords[3]]),
    )
    if not is_on_curve(parsed, b2):
        raise Groth16VerificationError(f"{label}_not_on_g2")
    if multiply(parsed, curve_order) is not None:
        raise Groth16VerificationError(f"{label}_not_in_g2_subgroup")
    return parsed
...
def real_circuit_gate_status(path: Path | None = None) -> dict[str, Any]:
    manifest_path = path or REAL_CIRCUIT_MANIFEST_PATH
    repo_root = manifest_path.parent.parent.parent if manifest_path == REAL_CIRCUIT_MANIFEST_PATH else manifest_path.parent.parent.parent
    blockers: list[str] = []
    manifest: dict[str, Any] = {}
    if not manifest_path.exists():
        blockers.append("real_circuit_manifest_missing")
    else:
        try:
            ...
            if isinstance(loaded, dict):
                ...
                blockers.append("real_circuit_manifest_not_object")
        except Exception as exc:
            blockers.append(f"real_circuit_manifest_unreadable:{type(exc).__name__}")
    required_true = (
        "r1cs_present",
        "wasm_present",
        "proving_key_present",
        "verifying_key_present",
        "trusted_setup_complete",
        "known_good_proof_passed",
        "tamper_proof_rejected",
        "verifier_key_substitution_rejected",
    )
    required_hashes = (
        "circuit_sha256",
        "r1cs_sha256",
        ...
        "tampered_proof_sha256",
        "substituted_verification_key_sha256",
    )
    path_hash_pairs = (
        ("circuit_path", "circuit_sha256"),
        ("r1cs_path", "r1cs_sha256"),
        ("wasm_path", "wasm_sha256"),
        ("proving_key_path", "proving_key_sha256"),
        ("verifying_key_path", "verifying_key_sha256"),
        ("proof_path", "proof_sha256"),
        ("public_inputs_path", "public_inputs_sha256"),
        ("synthetic_input_path", "synthetic_input_sha256"),
        ("tampered_proof_path", "tampered_proof_sha256"),
        ("substituted_verification_key_path", "substituted_verification_key_sha256"),
    )
    if manifest:
        if manifest.get("schema_version") != REAL_CIRCUIT_SCHEMA_VERSION:
            blockers.append("real_circuit_schema_mismatch")
        if manifest.get("circuit_id") != GROTH16_ENVELOPE_CIRCUIT_ID:
            blockers.append("real_circuit_id_mismatch")
        ...
        for path_key, hash_key in path_hash_pairs:
            ...
            if not artifact_path.exists():
                blockers.append(f"{path_key}_artifact_missing")
                continue
            if isinstance(expected_hash, str) and len(expected_hash) == 64:
                actual_hash = _sha256_file(artifact_path)
                if actual_hash != expected_hash:
                    blockers.append(f"{hash_key}_artifact_mismatch")
        if not any(blocker.endswith("_artifact_missing") or blocker.endswith("_artifact_mismatch") for blocker in blockers):
            try:
                def manifest_json(path_key: str) -> Any:
                    relative = manifest.get(path_key)
                    if not isinstance(relative, str):
                        raise Groth16VerificationError(f"{path_key}_missing")
                    return json.loads((repo_root / relative).read_text(encoding="utf-8"))

                verification_key = manifest_json("verifying_key_path")
                public_inputs = manifest_json("public_inputs_path")
                valid_payload = {
                    "proof": manifest_json("proof_path"),
                    "verifying_key": verification_key,
                    "public_signals": public_inputs,
                }
                valid_result = verify_groth16_payload(valid_payload)
                if valid_result.get("passed") is not True:
                    blockers.append("known_good_proof_verification_failed")
                tampered_result = verify_groth16_payload({
                    "proof": manifest_json("tampered_proof_path"),
                    "verifying_key": verification_key,
                    "public_signals": public_inputs,
                })
                if tampered_result.get("passed") is True:
                    blockers.append("tamper_proof_not_rejected")
                substituted_result = verify_groth16_payload({
                    "proof": manifest_json("proof_path"),
                    "verifying_key": manifest_json("substituted_verification_key_path"),
                    "public_signals": public_inputs,
                })
                if substituted_result.get("passed") is True:
                    blockers.append("verifier_key_substitution_not_rejected")
            except Exception as exc:
                blockers.append(f"real_circuit_dynamic_verification_failed:{type(exc).__name__}")
    blockers = sorted(set(blockers))
    return {
        "schema_version": "openencoder-real-groth16-circuit-gate-v1",
        "circuit_id": GROTH16_ENVELOPE_CIRCUIT_ID,
        "manifest_path": str(manifest_path),
        "passed": not blockers,
        "blocker_count": len(blockers),
        "blockers": blockers,
        "manifest_sha256": _sha256_json(manifest) if manifest else "",
    }
...
def build_envelope_statement(
    *,
    ...
    return {
        "schema_version": "openencoder-field-envelope-statement-v1",
        "circuit_id": GROTH16_ENVELOPE_CIRCUIT_ID,
...
def prove_envelope_statement(statement: dict[str, Any]) -> dict[str, Any]:
    ecc = _load_py_ecc()
    add = ecc["add"]
    multiply = ecc["multiply"]
    G1 = ecc["G1"]
    ...
    payload = {
        "schema_version": GROTH16_ENVELOPE_PROOF_SCHEMA_VERSION,
        "proof_system": GROTH16_PROOF_SYSTEM,
        "curve": "bn254",
        "circuit_id": GROTH16_ENVELOPE_CIRCUIT_ID,
        ...
        "proving_system_receipt": {
            "trusted_setup_model": "deterministic reference setup for OpenEncoder field-envelope fixtures",
...
def verify_envelope_proof(payload: dict[str, Any], *, expected_statement: dict[str, Any] | None = None) -> dict[str, Any]:
    blockers: list[str] = []
    if payload.get("schema_version") != GROTH16_ENVELOPE_PROOF_SCHEMA_VERSION:
        blockers.append("envelope_proof_schema_mismatch")
